...
class UpgradeShop
{
    ...
    public static function handleTransaction($formId, Player $player, BedWars $plugin): void
    {
        ...
        $player->sendMessage(TextFormat::GREEN . "§e§lBED§8 >§r§e Upgraded!");
    ...
    public static function sendBuyPage($selectedUpgrade, Player $player, BedWars $plugin) : void{
        ...
        if(!$player->getInventory()->contains(Item::get(Item::DIAMOND, 0, $cost))){
            $formData['content'].="\n" . TextFormat::RESET . TextFormat::RED . "Need " . $cost . " diamonds!";
        }elseif($playerTeam->getUpgrade($upgradeData['identifier']) < self::MAX_LEVELS[$upgradeData['identifier']]){
            $formData['content'].="\n" . TextFormat::RESET . TextFormat::GREEN . "Tap to buy!";
        }else{
            $level = $playerTeam->getUpgrade($upgradeData['identifier']) > 1 ? " " . TextFormat::YELLOW . "Level: " . TextFormat::WHITE . $playerTeam->getUpgrade($upgradeData['identifier']) : "";
            $formData['content'].="\n" . TextFormat::RESET . TextFormat::GREEN . "ACTIVATED!" . $level;
        }
