...
class ItemShop
{
    ...
    public static $shopWindows = [
        1 => ["name" =>"§l§eARMOR", "image" => "https://www.spigotmc.org/attachments/ftiroac-png.241966/"],
        2 => ["name" =>"§l§eWEAPONS", "image" => "http://icons.iconarchive.com/icons/chrisl21/minecraft/512/Stone-Sword-icon.png"],
        3 => ["name" =>"§l§eBLOCKS", "image" => "https://d1u5p3l4wpay3k.cloudfront.net/minecraft_gamepedia/0/07/White_Wool.png"],
        4 => ["name" =>"§l§eBOWS", "image" => "https://lh3.googleusercontent.com/MS2w4Tlmw4azfmxVcT9o29cq74YgMau26xni5DiqbzpxHFMIEpHtPxdGWWZlV-WD0oXNLUXsiKs2W2yAzMT0=s400"],
        5 => ["name" =>"§l§ePOTIONS" ,"image" => "http://www.blocksandgold.com//media/catalog/product/cache/3/image/200x/6cffa5908a86143a54dc6ad9b8a7c38e/s/p/splash_red.png"],
        6 => ["name" =>"§l§eSPECIALS", "image" => "https://d1u5p3l4wpay3k.cloudfront.net/minecraft_gamepedia/1/1e/TNT.png"]
    ];
    ...
    public static $itemData = [
        ...
        3 => [0 => ["name" => "Bow 1", "type" => self::PURCHASE_TYPE_GOLD, "amount" => 1, "price" => 12, "item" => ["id" => Item::BOW, "damage" => 0]],
            ...
            2 => ["name" => "Bow 3", "type" => self::PURCHASE_TYPE_EMERALD, "amount" => 1, "price" => 3, "item" => ["id" => Item::BOW, "damage" => 0]]
        ],
    ...
    public static function handleTransaction(int $id, $data, Player $p, BedWars $plugin){
        ...
        if(!$check){
            $p->sendMessage("§e§lBED§8 >§r§c You don't have enough§6 " . strtolower(ucfirst($typeString)) . " §to purchase this item!");
            return;
        }
        ...
        foreach($p->getInventory()->getContents() as $index => $content){
            if(self::isSword($content->getId()) && self::isSword($id)) {
                ...
                if ($id !== $content->getId()) {
                    ...
                    $p->sendMessage("§e§lBED§8 >§r§c You already have this sword!");
                    return;
                }
            }
        }
        $p->sendMessage("§e§lBEDW§8§8 >§r§e You have sucesfully purchased §6" . $itemData['name'] . " §efor §6" . $price . " " .  ucfirst($typeString));
        if($wasPurchased){
            return;
        }
    ...
    public static function handleBowTransaction(int $data, Item $item){
        switch ($data){
            case 1;
                ...
                break;
            case 2;
                $enchantment = new EnchantmentInstance(Enchantment::getEnchantment(Enchantment::POWER), 2);
                $enchantment1 = new EnchantmentInstance(Enchantment::getEnchantment(Enchantment::PUNCH), 3);
